    ...
    try{
        ...
        const expenses=await db.transaction.aggregate({
            where:{userId:user.id,type:"EXPENSE",
                date:{gte:startOfMonth,lte:endOfMonth}, // gte-Greater than , lte-lesser than 
                accountId
            },
            _sum:{
                amount:true
        ...
        console.error("Error fetching in budget",err)
        throw err
    }
}
...
export async function updateBudget(amount) {
    try{
        ...
        const budget=await db.budget.upsert({ // updates the budget of an existing one or create a new budget
